...
def get_move(board, depth):
    ...
    if opening_move:
        logger.info(f"Playing book move 📚: {opening_move}. Opening={opening_name}, phase={game_phase}")
        return opening_move
    ...
    logger.info(f"Playing engine move 🤖: {top_move}, eval={top_eval / 100.0}, phase={game_phase}")
    return top_move
